...
static void getSchannelClientHandle(PCredHandle ppClientCred) {
  ...
  char* ch = const_cast<char*>(UNISP_NAME_A);
...
static int tls_connect(tls_socket* s, const char* hostname, unsigned short port)
{
    ...
    //    const char* str =  UNISP_NAME_A;
    //        auto status = AcquireCredentialsHandleA(
    //            NULL, const_cast<char*>(str), SECPKG_CRED_OUTBOUND, NULL, &cred,
    //            NULL, NULL, &s->handle, NULL);
    //    if (status != SEC_E_OK)
    //    {
    //        closesocket(s->sock);
    //        WSACleanup();
    ...
    getSchannelClientHandle(&s->handle);
...
int tls_main()
{
    //const char* hostname = "10.10.45.51";
